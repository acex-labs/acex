...
def create_router(automation_engine):
    ...
    plug = automation_engine.inventory.regions
    for cap in plug.capabilities:
        func = getattr(plug, cap)
        response_model = get_response_model(func)
        path = plug.path(cap)
        method = plug.http_verb(cap)
        router.add_api_route(f"/regions{path}", func, methods=[method], response_model=response_model, tags=tags)
    ...
    router.add_api_route("/region_assignments/{id}", ram.delete_assignment, methods=["DELETE"], tags=tags)
