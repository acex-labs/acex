...
from acex_devkit.models.composed_configuration import SubInterface as SubInterfaceModel
...
class Interface(ConfigComponent):
    def _add_template(self):
        ...
        if template is not None:
            self.kwargs["interface_template"] = ReferenceTo(pointer=f"interface_templates.{template.name}")
    ...
    def _add_vrf(self):
        if self.kwargs.get("network_instance") is None:
            ...
            self.kwargs["network_instance"] = ReferenceFrom(
    ...
    def _helper(self):
        if self.kwargs.get("relay_helper") is not None:
            ...
            self.kwargs["relay_helper"] = ReferenceFrom(
                pointer=f"system.dhcp.relay.relay_servers.{helper.name}.interfaces"
            )

    def _lldp_enable(self):
        if self.kwargs.get("lldp_enabled"):
            self.kwargs["lldp"] = ReferenceFrom(pointer="lldp.interfaces")

    def _cdp_enable(self):
        if self.kwargs.get("cdp_enabled"):
