...
class NetflowConfigRecord(ConfigMap):
    def compile(self, context):
        netflow_global_config = NetflowGlobalConfig(
            name="netflow_global_config_123", enabled=True
        )
        ...
        svi123 = Svi(name="svi123", index=0, vlan=vlan123, network_instance=test_vrf)
        context.configuration.add(svi123)

        netflow_collector_1 = NetflowCollector(
            name="netflow_collector_123", cache_inactive=180, cache_active=360
        )
