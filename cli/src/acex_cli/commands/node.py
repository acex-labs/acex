...
def desired_config(
    ...
    if output == ConfigFormat.rendered:
        rendered = _get_rendered_config(sdk, node_id)
        typer.echo(rendered)
        return
    ...
    data = configuration.model_dump() if hasattr(configuration, "model_dump") else configuration
