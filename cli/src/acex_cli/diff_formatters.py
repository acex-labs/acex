...
from acex_devkit.configdiffer import ComponentDiffOp, Diff
                    ...
                    if show_values:
                        ...
                        if change.op == ComponentDiffOp.ADD and change.after_dict:
                            for attr_name, attr_value in change.after_dict.items():
                                ...
                                attr_label.append(
                                    formatted_val, style="dim green" if formatted_val == "null" else "green"
                                )
...
def print_diff_compact(diff, show_unchanged: bool = False):
    ...
    if diff.added:
        ...
        for change in diff.added:
            console.print(f"  [green]+ {change.component_type_name}:[/green] {change.component_name}")

    if diff.removed:
        console.print("\n[bold red]✖ Removed:[/bold red]")
        for change in diff.removed:
            console.print(f"  [red]- {change.component_type_name}:[/red] {change.component_name}")

    if diff.changed:
        console.print("\n[bold yellow]⚡ Modified:[/bold yellow]")
        for change in diff.changed:
            console.print(f"  [yellow]~ {change.component_type_name}:[/yellow] {change.component_name}")
        ...
        if change.changed_attributes:
            for i, attr in enumerate(change.changed_attributes):
                table.add_row(
                    ...
                    change.component_type_name if i == 0 else "",
